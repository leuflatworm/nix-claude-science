...
| 5 | `/bin` & `/usr/bin` are near-empty in the sandbox → no `sh`, `mkdir`, `tar`, `git`, ...; kernel setup and `edit_file` fail, and the control-plane operon-kernel hard-execs `/usr/bin/python3` (dies with exit 72, "interpreter /usr/bin/python3 not found") | shim binds a curated `buildEnv` (bash, coreutils, grep/sed/awk, tar/gzip/xz/zstd, git, curl, **python3**, ...) over `/bin` and `/usr/bin` (flake) |
